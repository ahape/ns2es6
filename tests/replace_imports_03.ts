namespace Brightmetrics.Admin.VoiceAnalytics.ViewModels {
    ...
    import MultipleSelectDialog = Brightmetrics.MultipleSelect.ViewModels.MultipleSelectDialog;
    import Constants = Brightmetrics.Admin.VoiceAnalytics.Constants;

    const showDescriptionLSKey = Constants.showDescriptionLSKey + ":profiles";
    ...
    export class ManageProfiles extends Brightmetrics.ViewModel {
        ...
        public profiles: KnockoutObservableArray<VAIs.IProfile>;
    ...
    class ProfileEditor extends Brightmetrics.ViewModel {
        ...
        private _initialState: VAIs.IProfile;
        ...
        private _createKoPossibleTargetingRuleFilters(props: IProfileEditorProps)
            : KnockoutComputed<VAIs.ITargetingRuleSpec[]> {
        ...
        private _updateTagCategories(allTagCategories: VAIs.ITagCategory[]): Promise<VAIs.ITagCategory[]> {
            ...
            const dialog: MultipleSelectDialog<string> = new MultipleSelectDialog<string>({
                listFetcher: () => $.Deferred().resolve({ data: allTagCategories })
                    .then((response: { data: VAIs.ITagCategory[]; }) => {
                        return response.data
    ...
    interface ITargetingRuleProps {
        targetDcg: number;
        targetingRule: VAIs.ITargetingRule;
        possibleTargetingRuleFilters: KnockoutComputed<VAIs.ITargetingRuleSpec[]>;
    }
        ...
        private _addFilterDialog(props: ITargetingRuleProps): Brightmetrics.ViewModels.DialogViewModel {
            ...
            const dialog: MultipleSelectDialog<string> = new MultipleSelectDialog<string>({
                listFetcher: () => $.Deferred().resolve({ data: props.possibleTargetingRuleFilters.peek() })
                    .then((response: { data: VAIs.ITargetingRuleSpec[]; }) => {
                        return response.data
                            .map((trfs, i) => ({
                                label: trfs.name,
                                value: trfs.id,
    ...
    interface ITargetingRuleFilterProps {
        targetDcg: number;
        filter: VAIs.ITargetingRuleFilter;
        possibleTargetingRuleFilters: KnockoutComputed<VAIs.ITargetingRuleSpec[]>;
    ...
    class TargetingRuleFilter extends Brightmetrics.ViewModel {
        ...
        toJSON(): VAIs.ITargetingRuleFilter {
            return {
    ...
    class TagCategory extends Brightmetrics.ViewModel {
        ...
        private _loadRuleSetTags() {
            return Utils.ajaxRequest(`/VoiceAnalytics/${defCompanyId}/TagCategories/${this.id}/RuleSetTags`)
                .then((response: { success: boolean; ruleSetTags: VAIs.IRuleSetTag[] }) => {
                    if (response.success) {
